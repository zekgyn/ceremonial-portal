...
class BookingController extends Controller
{
    ...
    {
        $bookings = Booking::whereHas('event', fn($q) => $q->where('user_id', auth()->id()))
            ->with(['event', 'vendorPackage.vendor'])
            ->latest()
            ->get();

        return Inertia::render('Client/Bookings/Index', compact('bookings'));
    }
